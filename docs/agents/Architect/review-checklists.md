...
- 是否补了必要注释，但没有写废话注释
- 是否避免引入新的隐式耦合
- 是否通过 `python3 script/check_code_health.py --check --base-ref <base-ref>`
- 是否新增了超过 400 行的 Swift 文件，或让历史超限文件超过锁定额度
- 是否擅自新增/提高 `script/code_health_baseline.json` 例外，或移除了源码扫描根目录
- 是否为单次使用逻辑新增了不必要的协议、包装层、转发壳或预留抽象
- 是否通过压缩排版规避行数，或为了拆文件而放宽 `private`、破坏职责边界
