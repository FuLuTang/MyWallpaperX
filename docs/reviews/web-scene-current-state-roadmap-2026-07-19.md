...
Web 已经具备可持续回归的正式运行主链。文件属性类型推断、强 DOM/视觉/交互证据、纹理 WebGL 的 loopback 路由、旧式颜色数组兼容、空 `file:///` 占位符处理、境外远程样式降级恢复、真实 64+64 双声道频谱及 Wallpaper Engine 兼容幅度响应、固定样本矩阵和切换/停止释放门禁均已落地。
...
剩余边界是设备和系统状态：还需验证真正的系统静音、输出设备切换、蓝牙重连、睡眠恢复，以及把 JS 频带与最终画面响应做自动时间对齐。这些不再是当前固定假波形的已知根因，但仍属于发布级系统矩阵。
...
尚未覆盖锁屏/解锁、休眠/唤醒、Space 切换、显示器增删、分辨率变化、Web/Video/Scene 快速互切、音频设备变化、系统代理启停、一般网络断开恢复和连续 WebContent 崩溃。远程字体的可重复失败/恢复已关闭，最新 34 项长批次也已全绿，但这些更宽的系统状态仍使生命周期只能算部分关闭。
...
| 多屏与生命周期 | 15 | 13 | 切换/stop/释放、音频需求启停和 34 项长批次通过；系统状态和屏幕热插拔未闭环 |
...
| **合计** | **100** | **89** | **已知样本与真实音频主链已闭环，系统状态、长期性能和发布流程仍未闭环** |

### 4.2 剩余工作量的正确理解

剩余不是“再补 11% 兼容代码”，而是完成以下 3 个工作包：
...
- 已完成 Web-to-Web 切换、loopback 启停、WKWebView 释放、Web 音频需求启停和最终 stop 零状态门禁。
- 最新 34 项长批次为 34A，远程字体硬失败/恢复和音频到无音频再到音频序列也通过；下一步完成系统状态矩阵、Web/Video/Scene 互切、30 分钟交互运行和 2 小时 soak。
...
Web 的运行主链已从“基本可用”推进到“有固定、作者源码、Steam CDN、完整四层兼容门、远程网络降级门、真实音频证据和释放门”。10 项固定门、5 项作者源码门、3 项 Steam CDN 门和 34 项完整门当前全绿；Google Fonts 在国内无直连或代理失效时不再阻塞启动，Web 音频也已从重复的桌面假波形改为按需 signed stereo FFT，并补回旧样本依赖的兼容幅度响应。因此现在可以声明“当前 34+5+3 已知样本功能兼容闭环，Web 音频宿主主链闭环”，仍不能声明“发布级最终完全闭环”或“以后所有样本都会成功”。后续不应继续以新增样本特判为主，而应集中完成系统状态/设备变化、长期资源预算、文件授权产品回归和发布流程接入。当前专用 WKWebView 宿主、受控资源协议、按需 loopback 和结构化诊断路线应继续保留，不应改回宽权限 `file://` 或引入重复宿主。
