...
Web 已经具备可持续回归的正式运行主链。文件属性类型推断、强 DOM/视觉/交互证据、纹理 WebGL 的 loopback 路由、旧式颜色数组兼容、空 `file:///` 占位符处理、境外远程样式降级恢复、真实 64+64 双声道频谱及 Wallpaper Engine 兼容幅度响应、固定样本矩阵、切换/停止释放门禁和重叠系统中断恢复门均已落地。
...
当前本机 34 个 Web 样本已固化为完整基线。采用本轮最终 Debug 构建运行的最新长批次为 **34 个 A，平均 98.2，证据覆盖 93.3%，完整门通过**。此前 `3763370103` 的失败已纠正归因：根因是 Google Fonts 在境外网络不可达或代理失效时同步阻塞启动，不是 `mwx-local` scheme task 未交接；现在远程样式会先降级、再限界重试，本轮同一长批次中该样本为 98 / A。音频生命周期序列 `audio -> non-audio -> audio -> stop` 也通过：监听需求分别触发采集启动、停止、重启和最终释放，3/3 `host.ready`、3/3 旧 `WKWebView` 释放，最终宿主状态为 0。新增系统状态门使用 `1509243786` 和循环 `afplay`，按 `system sleep -> display sleep -> system wake -> display wake -> lock -> unlock -> stop` 验证重叠原因锁存；`host.ready` 后 3 次采集均取得非静音 PCM，部分唤醒未提前重启，最终资源归零。

Web 目前没有已确认的宿主 P0 阻断，当前 34+5+3 已知样本的功能兼容主链和系统音频到 JS 主链可以视为闭环，但仍不能称为发布级“最终完全闭环”。确定性锁屏与重叠休眠恢复已关闭；尚缺真实 OS 电源周期、物理音频设备变化、其余系统状态、性能和长期运行预算、真实文件授权 UI 回归，以及 CI/发布流程接入。按本文八项能力门重算，当前工程成熟度约为 **90/100**。剩余 10 分不是 10% 兼容代码或剩余工时；工作主要集中在 3 个验证和产品化工作包，而不是继续堆兼容脚本。
...
前序专项报告保存在 `.codex/web-closure-final-20260720/`；本轮最终作者源码、Steam CDN、34 项完整门和系统中断门报告分别保存在 `.codex/web-external-final-20260720/results/`、`.codex/web-steam-final-20260720/results/`、`.codex/web-full-final-20260720/results/` 和 `.codex/web-system-state-final-20260721/results-pass2/`，Steam 样本副本保存在 `.codex/web-steam-representative-samples-20260720/`。这些目录被 Git 忽略，只作为本地复核证据保留到分支合并，不替代仓库内的矩阵定义和生产测试。
...
确定性系统状态恢复已关闭：消费者需求与实际采集资源已经分离；暂停、系统睡眠、显示器休眠或锁屏会释放 CoreAudio tap，最后一个中断原因结束后再由统一播放策略恢复；失败保留需求并限界退避。系统状态门还要求每一代恢复后出现非静音 PCM 数据，不能只以 CoreAudio 对象创建成功代替数据恢复。剩余边界是实际 OS 睡眠/唤醒、真正的系统静音、输出设备切换、蓝牙重连，以及把 JS 频带与最终画面响应做自动时间对齐。
...
| 媒体与音频 | 10 | 9 | signed stereo 64+64、真实声音相关性、按需生命周期和确定性中断恢复已验证；物理设备切换、系统静音和真实 OS 睡眠未完成 |
...
| **合计** | **100** | **90** | **已知样本、真实音频主链和确定性中断恢复已闭环，真实设备/OS、长期性能和发布流程仍未闭环** |

### 4.2 剩余工作量的正确理解

剩余不是“再补 10% 兼容代码”，而是完成以下 3 个工作包：

1. **系统生命周期与异常恢复**：确定性睡眠/锁屏已完成；继续覆盖真实 OS 电源周期、Space、屏幕热插拔、runtime 互切、一般网络/系统代理变化、物理音频设备变化和连续崩溃；不得以静默重试掩盖首轮失败。
...
```text
已完成：文件属性 -> 三源视觉/动态证据 -> 远程字体失败恢复 -> signed stereo 真实音频 -> 固定/作者源码/Steam CDN/34 项完整门 -> 按需采集与 stop 释放 -> 重叠睡眠/锁屏确定性恢复门
下一步：CoreAudio 配置失效、真实 OS/设备变化和 runtime 互切矩阵
  -> 性能、泄漏和功耗预算
...
- 已完成 Web-to-Web 切换、loopback 启停、WKWebView 释放、Web 音频需求启停、重叠睡眠/锁屏恢复和最终 stop 零状态门禁。
- 最新 34 项长批次为 34A，远程字体硬失败/恢复、音频到无音频再到音频序列和确定性系统中断恢复均通过；下一步完成 CoreAudio 配置失效、真实 OS/设备状态、Web/Video/Scene 互切、30 分钟交互运行和 2 小时 soak。
...
- 已完成可控双声道真实音频到 JS bin 的频率、幅度、声道和确定性睡眠/锁屏恢复回归；后续补物理设备切换、真正系统静音、实际 OS 睡眠及最终画面时间对齐。
...
Web 的运行主链已从“基本可用”推进到“有固定、作者源码、Steam CDN、完整四层兼容门、远程网络降级门、真实音频证据、确定性系统中断恢复门和释放门”。10 项固定门、5 项作者源码门、3 项 Steam CDN 门和 34 项完整门当前全绿；Google Fonts 在国内无直连或代理失效时不再阻塞启动，Web 音频也已从重复的桌面假波形改为按需 signed stereo FFT，并补回旧样本依赖的兼容幅度响应。因此现在可以声明“当前 34+5+3 已知样本功能兼容闭环，Web 音频宿主主链及确定性睡眠/锁屏恢复闭环”，仍不能声明“发布级最终完全闭环”或“以后所有样本都会成功”。后续不应继续以新增样本特判为主，而应集中完成物理设备与真实 OS 状态、runtime 互切、长期资源预算、文件授权产品回归和发布流程接入。当前专用 WKWebView 宿主、受控资源协议、按需 loopback 和结构化诊断路线应继续保留，不应改回宽权限 `file://` 或引入重复宿主。
